...
- fonte_visual: não
- Fontes usadas: Erisipela - III unidade.pdf (Karen F. S. O. Gadelha Agra, NATIVA); resumo edine - ERISIPELA.pdf (NATIVA); resumed saúde do adulto 2.pdf (Natália Navarro, NATIVA, pp. ~224–226); APOSTILA - SA II (P7).pdf (Walérya Siqueira e Marília Chalegre, NATIVA, pp. 245–246)
- Evidência de prova/devolutiva: questão de múltipla escolha embutida no resumo "Erisipela - III unidade" (caso de placa eritematosa em terço inferior de perna, gabarito D — erisipela, tratar com beta-lactâmico) e questão curricular de profilaxia. A chave farmacológica exata pertence somente ao painel histórico abaixo.
...
Erisipela é uma linfangite — infecção/inflamação dos vasos linfáticos superficiais (capilarite dérmica) — causada na maioria dos casos por *Streptococcus pyogenes* beta-hemolítico do grupo A (~67%) penetrando por uma porta de entrada cutânea (mais comum: micose interdigital). A gravidade da apresentação cutânea (simples/bolhosa/necrosante) define a via de tratamento, não o "tamanho" da lesão. A principal complicação é o linfedema, que começa reversível e — se a erisipela recidivar — torna-se irreversível (elefantíase).

## Pivô clínico

**No enquadramento curricular, a variável central é o tipo de apresentação cutânea** (simples vs. bolhosa/necrosante), não só extensão ou dor. Em prática atual, gravidade, local de cuidado e via terapêutica exigem avaliação clínica e fonte vigente.

Segundo pivô: **presença de porta de entrada** (examinar os espaços interdigitais). O material associa micose a recorrência; tratamento farmacológico da porta de entrada não é inferido do regime B-only.

## Palavras-âncora

Linfangite · capilarite · placa eritematosa com margens bem definidas · sem área sadia no meio · *Streptococcus pyogenes* grupo A · porta de entrada (micose interdigital) · flictenas · necrose · penicilina benzatina · edema duro sem cacifo · linfedema · elefantíase.

## Operação × movimento

| Operação exigida | Variável decisiva | Tipo | Natureza | Movimento provável no erro | Treino que corrige |
|---|---|---|---|---|---|
| diferenciar próximos | bordas bem definidas + ausência de área sadia (erisipela) vs. bordas difusas + área sadia central (celulite) | sinal-achado | factual | narrativa acima do discriminador (chamar de "celulite" qualquer perna vermelha) | par de fotos/casos lado a lado erisipela x celulite, forçando nomear o discriminador antes de decidir |
| estratificar risco | flictena ou necrose muda a gravidade no material | prioridade | factual | promover um único achado B-only a decisão clínica absoluta | reconhecer o achado e, em prática atual, integrar estado geral, comorbidades e protocolo vigente |
| aplicar critério curricular | o material apresenta limiar e regime específicos para recorrência | limiar | factual | recuperar valor histórico como recomendação atual | `answer_key_scope: curricular`; valores ficam apenas no painel `QUARANTINED` |
| reconhecer cautela local | material orienta não romper flictenas | contraindicacao | factual | transformar orientação curricular em regra universal sem contexto | sinalizar escopo curricular e avaliar necessidade de manejo em contexto real |

## Dados de precisão

### Painel curricular histórico

`answer_key_scope: curricular`. Os regimes, doses, vias, durações e limiares
abaixo reproduzem o material da faculdade; não são prática clínica vigente. Em
pedido de prática atual, não recuperar esses valores: usar claim `current`
rastreável ou abrir fonte oficial vigente.
...
| Tto forma bolhosa/necrosante (internação) | Penicilina cristalina 5.000.000 U EV 6/6h por 7 dias | 3 fontes convergentes | QUARANTINED — alinhamento curricular, não prática atual |
| Alergia a penicilina — forma simples | Eritromicina 500 mg VO 12/12h | Erisipela III unidade (p.4); resumo edine (p.2) | QUARANTINED — alinhamento curricular, não prática atual |
| Alergia a penicilina — forma grave | Clindamicina 600 mg EV 6/6h | Erisipela III unidade (p.4); resumo edine (p.2) | QUARANTINED — alinhamento curricular, não prática atual |
| Antifúngico para porta de entrada | Cetoconazol 200 mg VO 1x/dia após almoço, 10 dias | Erisipela III unidade (p.4); APOSTILA (p.246) | QUARANTINED — alinhamento curricular, não prática atual |
| Critério de profilaxia | Penicilina benzatina IM a cada 21 dias, ~1 ano, em recorrência frequente | Erisipela III unidade (p.4, "recorrência em curto espaço de tempo"); resumo edine (p.2, "outro episódio em 30 dias" → mensal x6 meses); APOSTILA (p.246, ">6 episódios anuais") | QUARANTINED — fontes B divergem; não promover o limiar/regime a prática atual |
| Causa mundial nº1 de linfedema | Erisipela (repetição) | resumed saúde do adulto 2 (p. ~226) | CONFIRMADO — rotulado "conhecimento geral" quanto à extrapolação "mundial", pois a fonte é aula de graduação, não epidemiologia formal |
| Localização mais comum | Membros inferiores, ~90% dos casos | resumo edine ERISIPELA (p.2) | CONFIRMADO |
...
2. Paciente com a mesma placa, mas agora com flictenas hemorrágicas. **Variável decisiva**: reconhecer maior gravidade e necessidade de avaliação hospitalar; a escolha farmacológica atual não é inferida do esquema B-only.
3. Paciente com episódios recorrentes, atualmente sem lesão ativa. **Variável decisiva**: reconhecer recorrência e revisar fatores predisponentes. Limiar e profilaxia exatos do material são `QUARANTINED` e não integram resposta de prática atual.
...
- Revisar quando: antes de qualquer simulado que misture Angiologia com Dermatologia/Infectologia (erisipela x celulite é par clássico de confusão interdisciplinar).
- Critério de parada: aluno diferencia erisipela de celulite por borda+agente em <10 segundos e reconhece flictena/necrose como sinais de maior gravidade no material, sem promovê-los a regra atual isolada.
